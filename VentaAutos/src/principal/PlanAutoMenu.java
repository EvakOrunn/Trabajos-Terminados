...
public class PlanAutoMenu {
    ...
    public static void listar() {
        int contador = 0;
        int caracteres = 51; // Caracteres de la tabla. Cambiar para ajustar ancho de lineas
        System.out.println(Consola.repeat("-", caracteres)); // Se puede llamar a un metodo sobre el string ya que es un objeto
        System.out.println("| Cod. Plan | Cod. Vend. | Grupo | Orden |  Plan  |");
    ...
    public static void listarPlanAutoPorVendedor() {
        
        if (VendedorMenu.cantidad() == 0) {
            System.out.println("No hay Vendedore disponibles.");
            Consola.pausa();
            return;
        }
    ...
    public static Registro leer() {
        PlanAuto pl = new PlanAuto();
        int codigo;
        do {
            System.out.print("Codigo Plan Auto--> ");
            codigo = Consola.readInt();
            if (obtener(codigo) != null) {
                System.out.println("Codigo ya existente.");
                codigo = -1;
            }
        } while (codigo < 0 || codigo > pl.tamArchivo());
        try {
            pl.setCodigoSuscripcion(codigo);
        } catch (NumeroNegativoExcepcion ex) {
            Logger.getLogger(VuelosMenu.class.getName()).log(Level.SEVERE, null, ex);
        }
        pl.cargarDatos();
        Registro reg = new Registro(pl, pl.getCodigoSuscripcion()); // Aqui es donde se indica que la clave principal es "Codigo"
        return reg;
    }

    public static PlanAuto obtener(int codigo) {
        m2.abrirParaLectura();
        ...
        PlanAuto pl = (PlanAuto) reg.getDatos();
        m2.cerrarArchivo();
    ...
    public static void modificar() {
        ...
        Vendedor pl = (Vendedor) reg.getDatos();
        pl.cargarDatos();
        reg.setDatos(pl);
    ...
    public static void inicializarArchivo() {
        try {
            m2 = new Archivo("PlanAuto.dat", new PlanAuto());
            if (!m2.getFd().exists()){
                m2.crearArchivoVacio(new Registro(new PlanAuto(), 0));
            }
    ...
    public static void menu() {
        ...
        do {
            ...
            System.out.println("----------------------------------");
            System.out.println("- 1. Alta de planes              -");
            System.out.println("- 2. Baja de planes (lógica)     -");
            System.out.println("- 3. Modificacion de planes      -");
            System.out.println("- 4. Listar planes activos       -");
            System.out.println("- 5. Listar planes por destino   -");
            ...
            switch (op) {
                ...
                case 0:
                    System.out.print("\033[H\033[2J");
                    System.err.println("Menu Cerrado");
                    Consola.pausa();
                    System.out.print("\033[H\033[2J");
                    break;
            }
