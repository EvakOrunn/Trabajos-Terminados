...
public class InventoMenu {
    ...
    public static void listar() {
        int contador = 0;
        int caracteres = 64; // Caracteres de la tabla. Cambiar para ajustar ancho de lineas
        System.out.println(Consola.repeat("-", caracteres)); // Se puede llamar a un metodo sobre el string ya que es un objeto
        System.out.println("| Cod. Inventor | Cod. Invento | Patente | Monto | Denominacion |");
    ...
    public static void listarPorDestino() {
        ...
        if (InventorMenu.cantidad() == 0) {
            System.out.println("No hay Inventores disponibles.");
            Consola.pausa();
            return;
        }
        ...
        int codigoInventor;
        do {
            System.out.print("Codigo de Inventor--> ");
            codigoInventor = Consola.readInt();
        } while (InventorMenu.obtener(codigoInventor) == null); // Repetir hasta que se encuentre destino
        ...
        while (!m2.eof()) {
            ...
            if (reg.getActivo()) { // Pregunto si esta activo
                Invento v = (Invento) reg.getDatos(); // Obtengo los datos
                if (v.getCodigoInventor() == codigoInventor) { // Pregunto si es hacia el destino que quiero
    ...
    public static Registro leer() {
        ...
        try {
            vuelo.setCodigoInvento(codigo);
        } catch (NumeroNegativoExcepcion ex) {
            Logger.getLogger(VuelosMenu.class.getName()).log(Level.SEVERE, null, ex);
        }
        vuelo.cargarDatos();
        Registro reg = new Registro(vuelo, vuelo.getCodigoInvento()); // Aqui es donde se indica que la clave principal es "Codigo"
        return reg;
    }
    ...
    public static void cargar() {
        ...
        try {
            ...
            System.out.println("Error al cargar el archivo Invento: " + e.getMessage());
            System.exit(1);
        }
        m2.cerrarArchivo();
    }
    ...
    public static void inicializarArchivo() {
        try {
            m2 = new Archivo("Invento.dat", new Invento());
            if (!m2.getFd().exists()){
    ...
    public static void menu() {
        ...
        do {
            System.out.println("-----------------------------------");
            System.out.println("          MENU DE INVENTO          ");
            System.out.println("-----------------------------------");
            System.out.println("- 1. Alta de invento              -");
            System.out.println("- 2. Baja de invento (lógica)     -");
            System.out.println("- 3. Modificacion de invento      -");
            System.out.println("- 4. Listar invento activos       -");
            System.out.println("- 5. Listar inventos por inventor -");
